...
class AgentEnv(gym.Env):
    def __init__(self):  
        ...
        self.count = 0
        self.action_space = gym.spaces.Box(-10,10, shape=(2,), dtype=np.float32)
        self.action = 0
        self.reward = 0
        ...
        # self.observation_space = gym.spaces.Box(np.array([0,0,0,0,0,0]),np.array([self.width,self.height,self.width,self.height, self.width, self.height]))
        self.observation_space = gym.spaces.Box(np.array([0,0,0,0,0,0]),np.array([self.width,self.height,self.width,self.height,self.width,self.height]))
    ...
    def step(self,action):        
        ...
        if self.player.colliderect(self.target_image):
            ...
            self.reward += -(np.sqrt((self.player.x-self.target_pos[0])**2 + (self.player.y-self.target_pos[1])**2))*0.01
            done = False
        ...
        state1 = np.array([self.player.x,self.player.y,self.target_pos[0],self.target_pos[1],self.obstacle1_pos[0],self.obstacle1_pos[1]])
    ...
    def reset(self):
        ...
        return np.array([self.player.x,self.player.y,self.target_pos[0],self.target_pos[1],self.obstacle1_pos[0],self.obstacle1_pos[1]])
    ...
    def render(self,_):
        ...
        pos_text = font.render(f'player_pos: {[self.player.x,self.player.y]}',True,(0,255,0))
...
gym.register(
    id = 'AgentEnv-v0',
    entry_point= 'env_continuos:AgentEnv',
